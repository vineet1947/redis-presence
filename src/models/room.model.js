...
const { maxParticipants } = require("../config/service.config");
...
exports.roomModel = class extends colyseus.Room {
  ...
  async isClientOnline(clientId) {
    try {
      console.log("i am here ..");
      const isClientOnline = await this.presence.get(clientId);
  ...
  onCreate(options) {
    ...
    const isClientOnline = isClientOnline(clientId);

    this.onMessage("mute", (client, message) => {
      clients[client.sessionId].mute = message.mute;
      this.broadcast("mute", clients);
      this.presence.set("mynewkey", "vin44");
      const playerkey = isClientOnline("mynewkey");
      console.log("final :: ", playerkey);
    });

    this.onMessage("mutePlayer", (client, message) => {
      this.clients
        .filter((client) => client.sessionId === message.clientId)
        .map((client) => {
    ...
    this.onMessage("streamNewClient", (client, message) => {
      const sClient = this.clients.filter(
        (client) => client.sessionId == message
      )[0];
    ...
    this.onMessage("loadComplete", (client) => {
      this.broadcast("join", clients);
      this.broadcast("userJoined", clients[client.sessionId].givenName, {
        except: client,
      });
    });
    ...
    this.onMessage("sit", (client, message) => {
      ...
      Object.keys(sittables)
        .filter((sittable) => message.name != sittable)
        .map((sittable) => {
